...
This repo is a thin MCP server adapter over `crea-ddf`. It should expose useful MCP tools and resources by calling the SDK instead of rebuilding CREA DDF clients, schema definitions, database access, sync jobs, or migrations locally.

- Import API-level SDK capabilities from `crea-ddf` and DB-backed query capabilities from `crea-ddf/db`.
- Keep the MCP layer focused on transport, tool input schemas, result shaping, and small orchestration around SDK calls.
- Do not copy `crea-ddf` migrations, Drizzle schema, generated metadata, or OData/query builders into this package.
- Use `DdfDatabase.layerConfig` and `DdfDbClient.layer` for synced Postgres tools; `DATABASE_URL` is the required runtime config for those tools.
- Add `@modelcontextprotocol/node` only when a Streamable HTTP transport is implemented. The default starter transport is stdio from `@modelcontextprotocol/server`.
- Prefer adding a missing data capability upstream in `crea-ddf` over using raw SQL in this MCP package.
- Keep tests scoped to `src/**/*.test.ts`; `references/effect-smol` is local reference material and must stay excluded from Vitest runs.

The first useful tool surface should be narrow and high-signal:

- `ddf_list_properties` and `ddf_get_property`
- `ddf_list_open_houses`
- destination/sync inspection tools such as `ddf_list_destinations`
- API-backed tools only when they clearly need live CREA DDF API access rather than the synced database
...
5. Also inspect nearby repo code under `src` and installed `crea-ddf` declarations under `node_modules/crea-ddf/dist`.
...
- Prefix local MCP helpers with `DdfMcp` when they become reusable services.
- Keep SDK method names and data semantics from `crea-ddf`; MCP tool names should describe the exposed operation, such as `ddf_list_properties`.
